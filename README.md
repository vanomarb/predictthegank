...
`public/write-note.lottie.json` (a notepad-and-pencil loop, for `wrongModal` —
see public.js) was supplied directly rather than sourced from LottieFiles or
generated here; its license is unconfirmed, so treat it the same way as the
credited pair above if this project is ever redistributed.
...
- **The verdict**, when a window closes: a hit if a sighting was logged *for it*,
  ...
  verdict is held back until the trailing grace has elapsed, and is always judged
  against the server's own record of today's sightings — a background tab never
  makes the verdict itself wrong, only the badges' timing. The public tracker
  shows it as a modal with a
  ...
  The watcher that decides when to fire that modal/toast persists its "already
  reported" set to localStorage (per calendar day), not just an in-memory Set.
  A tab backgrounded for a while looks merely unfocused from the outside, but
  Chrome and Safari will both discard and later reload a hidden tab under
  memory pressure — the whole JS context, closures included, is gone. Without
  the persisted copy, that reload re-primes from scratch and treats any window
  that closed while the tab was away as "already seen" (the same rule meant for
  a genuine first load), so its hit/miss never surfaces — while the badge
  underneath, computed fresh from the server on every render, shows the right
  verdict anyway. The persisted set is what lets the reload pick up where the
  discarded page left off instead of silently swallowing it.
